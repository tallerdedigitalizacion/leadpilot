// Ciclo completo de la tarea Fargate del scraper de Google Maps (gosom/google-maps-scraper,
// imagen pública) dentro de una sola invocación — mismo patrón que analysis-worker
// (RunTask -> poll IP -> llamadas HTTP -> StopTask). Sin cambios de lógica respecto a la
// versión anterior de index.ts — solo se movió a su propio archivo.
...
function mapRows(rows: Record<string, string>[], extractEmails: boolean): ScrapedLead[] {
  ...
  for (const row of rows) {
    ...
    const email = extractEmails ? firstEmail(row.emails) : undefined;
    // Si se pidió extraer emails, un lead sin email nunca se puede contactar por el único
    // canal 1:1 que tiene el funnel — mejor saltarlo que gastar Fargate+Claude en un callejón
    // sin salida. Si no se pidió extraer emails, se ingresa igual (comportamiento manual de siempre).
    if (extractEmails && !email) continue;
    leads.push({
...
export async function runGosomScrape(job: ScrapeJob): Promise<ProviderResult> {
