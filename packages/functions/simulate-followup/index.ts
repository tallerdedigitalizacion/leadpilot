...
import { buildLinks, generateFollowupEmail } from '../shared/followup-email';
...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  ...
  let body: { followupNumber?: number };
  try {
  ...
  if (body.followupNumber !== 1 && body.followupNumber !== 2) {
    return respond(400, { error: 'followupNumber must be 1 or 2' });
  }
