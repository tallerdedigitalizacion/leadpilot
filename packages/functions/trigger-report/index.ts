import { APIGatewayProxyEventV2, APIGatewayProxyResultV2 } from 'aws-lambda';
import { DynamoDBClient } from '@aws-sdk/client-dynamodb';
import { DynamoDBDocumentClient, GetCommand, UpdateCommand } from '@aws-sdk/lib-dynamodb';
import { LambdaClient, InvokeCommand } from '@aws-sdk/client-lambda';

const ddb = DynamoDBDocumentClient.from(new DynamoDBClient({}), { marshallOptions: { removeUndefinedValues: true } });
const lambdaClient = new LambdaClient({});
const TABLE = process.env.LEADS_TABLE_NAME!;
const GENERATE_REPORT_FN = process.env.GENERATE_REPORT_FUNCTION_NAME!;

const ALLOWED_STATUSES = new Set(['ANALYZED', 'SENT', 'CALLED', 'RESPONDED', 'NO_RESPONSE']);

function respond(statusCode: number, body: unknown): APIGatewayProxyResultV2 {
  return {
    statusCode,
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(body),
  };
}

export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  const leadId = event.pathParameters?.leadId;
  if (!leadId) return respond(400, { error: 'leadId is required' });

  const result = await ddb.send(new GetCommand({ TableName: TABLE, Key: { leadId } }));
  if (!result.Item) return respond(404, { error: 'Lead not found' });

  const { status } = result.Item as { status: string };
  if (!ALLOWED_STATUSES.has(status)) {
    return respond(400, { error: `Lead must be ANALYZED or later. Current: ${status}` });
  }

  const now = Date.now();

  // Marcar como generando — visible en todos los tabs via polling
  await ddb.send(new UpdateCommand({
    TableName: TABLE,
    Key: { leadId },
    UpdateExpression: 'SET isGeneratingReport = :true, reportGenerationStartedAt = :now',
    ExpressionAttributeValues: { ':true': true, ':now': now },
  }));

  // Invocación asíncrona — API Gateway devuelve 202 inmediatamente
  await lambdaClient.send(new InvokeCommand({
    FunctionName: GENERATE_REPORT_FN,
    InvocationType: 'Event',
    Payload: JSON.stringify({ leadId }),
  }));

  return respond(202, { message: 'Report generation started', leadId });
};
